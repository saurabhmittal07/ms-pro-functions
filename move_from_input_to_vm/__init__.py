        ...
        for table in sync_table_list:
            with pyodbc.connect('DRIVER=' + ODBC_DRIVER + ';SERVER=tcp:' + server + ';DATABASE=' + database + ';UID=' + username + ';PWD=' + password) as conn:
                with conn.cursor() as cursor:
                    ...
                    # sql = sql.replace("${startDay}", str(startDay))
                    # sql = sql.replace("${endDay}", str(endDay))
                    sql = sql.replace("${partner}", str(partner))
                    sql = sql.replace("${store_name}", str(store_name))
                    # sql = sql.replace("${brand}", str(brand))
                    # sql = sql.replace("${category}", str(category))
                    ...
                    # sql = sql.replace("${gender}", str(gender))
                    # sql = sql.replace("${startDayS1}", str(startDayS1))
                    # sql = sql.replace("${endDayS1}", str(endDayS1))
                    #  sql = sql.replace("${startDayS2}", str(startDayS2))
                    # sql = sql.replace("${endDayS2}", str(endDayS2))
